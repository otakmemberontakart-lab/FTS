...
const calcResult = computeFullPrep({
  ...
  maxRunwayFt: depAirport.max_runway_ft
});
...
if (!html.includes(depAirport.icao)) problems.push('ICAO ADEP tidak muncul');
if (!html.includes(arrAirport.icao)) problems.push('ICAO ADES tidak muncul');
if (!html.includes('V1')) problems.push('bagian V-speed tidak muncul');

console.log('HTML length:', html.length, 'chars');
console.log(problems.length === 0 ? 'PASS — tidak ada undefined/NaN, semua section ada' : 'FAIL: ' + problems.join(', '));

fs.writeFileSync(new URL('../_preview_ofp_output.html', import.meta.url),
  `<!DOCTYPE html><html><head><meta charset="utf-8"><title>OFP Preview</title></head><body style="background:#222;padding:20px;">${html}</body></html>`);
console.log('Preview file written to _preview_ofp_output.html (buat inspeksi visual).');
